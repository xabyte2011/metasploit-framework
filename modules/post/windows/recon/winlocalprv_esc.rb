...
class Metasploit3 < Msf::Post
	...
	def initialize(info={})
		super(update_info(info,
			'Name'           => "Post Windows Recon Local Vulnerability Check",
			'Description'    => %q{
		...
		register_options(
			[
				OptPath.new('KBLIST', [true, 'List of Microsoft KB numbers to check', ::File.join(Msf::Config.install_root, "data", "post", "kblist.txt")])
			], self.class)
	end
	...
	def run
		...
		if /#{p2}/ == /SYSTEM/
			print_status("\t Note, you already have SYSTEM privileges")
		elsif adminbuff =~ /#{p2}/ 
			print_status("\t Note, you are already Administrator #{p2}")
		else
			print_status("\t You do NOT have Administrator privilege:- #{p2}")
		end
